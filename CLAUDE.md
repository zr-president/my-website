...
13. **图片/海报与内容强绑定（多模态）**：动漫/影视/购车等视觉板块的配图必须与推荐内容**一一对应**——真实作品（如 Re:Zero/死神/蜘蛛侠/无职转生/时光代理人）用 **TMDB 官方海报**（`image.tmdb.org/t/p/w500/{poster_path}.jpg`，可热链）；虚构/未来作品用**主题高契合的真实图**（Unsplash）。同步规范：①PICKS(首页/分类推荐) 与 DETAIL(详情页) 两处都要改，不能只改一处（曾出现详情页图没跟上首页数据的 bug）；②每板块图数量与内容条数匹配（不限 2 张）；③**每次内容更新时，若新增/替换了推荐作品，必须同步给它配上对应海报图**（img 字段=TMDB poster_path 或主题图），禁止出现推荐列表无图或图文不符；④购车等无 TMDB 条目的用真实车辆照片（Unsplash），并确认可热链后再用。
